...
export class UserService implements IUserService {
  ...
  async verifyCurrentToken(token: string) {
    try {
  ...
  private async verifyAppleToken(
    ...
    try {
      ...
      const { data: applePublicKey } =
        await axios.get<AppleAuthKeys>(APPLE_AUTH_TOKEN_URL);
  ...
  async signUpByApple(data: AppleLoginData): Promise<UserResponse | null> {
    try {
      const { email, name } = await this.verifyAppleToken(data.token);
